...
// export function Login(parentEl: HTMLElement) {
...
//   parentEl.innerHTML = `
//     <div id=" class="mx-auto w-min h-min my-10 border p-5 py-10 shadow-lg relative flex flex-col text-gray-700 bg-transparent  rounded-xl bg-clip-border">
...
//     signUpLink?.addEventListener("click", (_) => {
//       console.log("eeeee");
...
export class Login {
  ...
  private handleSubmit() {
    ...
    submitBtn?.addEventListener("click", async (_) => {
      try {
        console.log("hey");
        const response = await axiosInstance.post("/auth/login", this.state);
        const { accessToken, refreshToken } = response.data;
